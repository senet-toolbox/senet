...
const Null = Vapor.Null;

var search_box: Vapor.Binded = Vapor.Binded{};
var binded: Binded = .{};
var show: bool = false;
var current_index: usize = 0;
var dynamic_menu_items = Vapor.persist.Array(MenuItem);
var uuids = Vapor.persist.Array([]const u8);
pub fn init() void {
    dynamic_menu_items.appendSlice(menu_items);
...
fn search(evt: *Vapor.Event) void {
    ...
    for (menu_items) |item| {
        ...
        if (found_match) {
            dynamic_menu_items.append(item);
        }
    }
}
            ...
            Row()
                ...
                List().style(&.{
                    ...
                    for (dynamic_menu_items.items, 0..) |item, j| {
                        ...
                        var list_item: Vapor.Builder(.static) = ListItem();
                        uuids.append(list_item.getUUID());
                        list_item
                            ...
                            // Link(.{ .url = item.link, .aria_label = item.title })
                            //     .textDecoration(.none)
                            //     .body()({
                            Button(navigate, .{item.link})
                                .width(.full)
                                .layout(.left_center)
                                .direction(.column)
                                .pointer()
                                ...
                                Text(item.link).style(&.{
                                    .visual = .{ .font_size = 12, .text_color = text_color },
                                    .font_family = "IBM Plex Mono,monospace",
                                }).end();
                                // });
                                for (item.tags) |tag| {
                                    Link(.{ .aria_label = tag.sub_title, .url = tag.url }).style(&.{
                                        .layout = .left_center,
