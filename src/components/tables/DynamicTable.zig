...
//
pub fn DynamicTable(columns: []const []const u8, config: struct { actions: ?[]const Action(Value) = null }) type {
    ...
    return struct {
        ...
        const table_columns: []const []const u8 = columns;
        ...
        fn registerKeyboardNav(_: *Self) void {
            // OverlayManager.register(.keydown, handleDynamicTableKeyPress, table);
        }

        fn unregisterKeyboardNav(_: *Self) void {
                ...
                Box()
                    ...
                    Box()
                        ...
                        for (table_columns) |column| {
                            const entry = row.getEntry(column) orelse unreachable;
            ...
            for (table.data) |item| {
                ...
                inline for (columns, 0..) |col, i| {
                    ...
                    if (active_filters[i]) |filter_text| {
                        if (!matchesSearch(col, item, filter_text)) {
                            matches = false;
                            break;
                        }
                    }
        ...
        fn Pagination(table: *Self) void {
            ...
            std.log.info("filtered_data.items.len {d}", .{table.filtered_data.items.len});
