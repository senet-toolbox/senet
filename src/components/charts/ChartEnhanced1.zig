...
// /// Example: Create a Nightwatch-style stacked grouped bar chart
// pub fn exampleNightwatchChart(allocator: std.mem.Allocator) !*Chart {
//     var chart = try allocator.create(Chart);
//     chart.* = Chart.init(allocator, .{
//         .width = 800,
//         .height = 400,
//         .margin = .{ .top = 30, .right = 30, .bottom = 50, .left = 60 },
//         .background = Vapor.Types.Color.hex("#1a1a2e"),
//     });
//
//     // Define colors for each status code category
//     const gray = Vapor.Types.Color.hex("#6b7280"); // 1/2/3XX
//     const yellow = Vapor.Types.Color.hex("#f59e0b"); // 4XX
//     const red = Vapor.Types.Color.hex("#ef4444"); // 5XX
//
//     // Create data points with stacked segments
//     // Each point represents a time bucket
//     const data_1xx_2xx_3xx = [_]Chart.Point{
//         .{ .x = 1, .stack = &.{
//             .{ .value = 50, .color = gray },
//             .{ .value = 30, .color = gray },
//         } },
...
//     };
//
//     try chart.addSeries(.stacked_bar, "1/2/3XX", &data_1xx_2xx_3xx, .{ .color = gray, .bar_radius = 2 });
//     try chart.addSeries(.stacked_bar, "4XX", &data_4xx, .{ .color = yellow, .bar_radius = 2 });
//     try chart.addSeries(.stacked_bar, "5XX", &data_5xx, .{ .color = red, .bar_radius = 2 });
//
//     chart.xAxis(.{ .label = "Time", .tick_count = 4 });
//     chart.yAxis(.{ .label = "Requests", .tick_count = 5 });
...
// }
//
// /// Example: Diverging stacked bar (values going up AND down from baseline)
// pub fn exampleDivergingChart(allocator: std.mem.Allocator) !*Chart {
//     var chart = try allocator.create(Chart);
//     chart.* = Chart.init(allocator, .{
//         .width = 800,
//         .height = 400,
...
//     });
//
//     const green = Vapor.Types.Color.hex("#22c55e");
//     const red = Vapor.Types.Color.hex("#ef4444");
//
//     // Each point has positive (profit) and negative (loss) segments
//     const profit_loss_data = [_]Chart.Point{
//         .{
//             .x = 1,
//             .stack = &.{
//                 .{ .value = 100, .color = green }, // Profit stacks up
//                 .{ .value = -30, .color = red }, // Loss stacks down
//             },
//         },
//         .{ .x = 2, .stack = &.{
...
//     };
//
//     try chart.addSeries(.stacked_bar, "Profit/Loss", &profit_loss_data, .{ .bar_radius = 4 });
//
//     chart.xAxis(.{ .label = "Quarter", .tick_count = 4 });
//     chart.yAxis(.{ .label = "Amount ($)", .tick_count = 5 });
//
//     try chart.build();
//     return chart;
// }
