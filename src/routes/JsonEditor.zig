...
pub fn init() void {
    // text = @embedFile("ReverbMiddleware.zig");
...
fn onKeyDown(evt: *Vapor.Event) void {
    ...
    if (std.mem.eql(u8, key, "Enter")) {
        ...
        holding_meta = true;
    } else if ((std.mem.eql(u8, key, "/") or std.mem.eql(u8, key, "?")) and holding_meta) {
        // Toggle comment (Cmd+/ or Ctrl+/)
        evt.preventDefault();
        // toggleComment();
    } else if (std.mem.eql(u8, key, "d") and holding_meta) {
        // Duplicate line (Cmd+D or Ctrl+D)
        evt.preventDefault();
        duplicateLine();
    } else if (std.mem.eql(u8, key, "k") and holding_meta) {
        // Delete line (Cmd+K or Ctrl+K)
        evt.preventDefault();
        deleteLine();
    } else if (std.mem.eql(u8, key, "]") and holding_meta) {
        ...
        evt.preventDefault();
        outdentCurrentLine();
    } else if (std.mem.eql(u8, key, "ArrowUp") and holding_meta and evt.altKey()) {
        // Move line up (Cmd+Alt+Up)
        evt.preventDefault();
        moveLineUp();
    } else if (std.mem.eql(u8, key, "ArrowDown") and holding_meta and evt.altKey()) {
...
fn handleTab(shift: bool) void {
    ...
    if (selection.start != selection.end) {
        ...
        if (shift) {
            handleOutdent(selection.start, selection.end);
        } else {
            handleIndent(selection.start, selection.end);
        }
    } else {
        if (shift) {
...
fn handleIndent(start: usize, end: usize) void {
...
fn handleOutdent(start: usize, end: usize) void {
    // Find line boundaries
    const line_start = if (std.mem.lastIndexOf(u8, text[0..start], "\n")) |pos| pos + 1 else 0;
    const line_end = std.mem.indexOf(u8, text[end..], "\n") orelse text.len;
    const actual_end = end + line_end;
...
fn indentCurrentLine() void {
    const cursor_pos = text_area.cursorPosition();
    const line_bounds = getLineBounds(cursor_pos);
...
fn getLineBounds(cursor_pos: usize) LineBounds {
...
fn render() void {
    Vapor.Stack()
        ...
        Row()
            ...
            .layout(.center)
            .children({
            highlighter.renderErrors() catch unreachable;
        });
    });
}
