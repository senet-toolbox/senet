...
pub fn render(user: User) void {
    // Since we are returning a closure we first call the Box(Style{}) function with our styling as an argument.
    ...
    // First Box(Style{}) is called with Style{} as an argument, which returns a closure, fn (void) void;
    ...
    // This also works
    const closure = Box(Style{}); // fn (void) void
    closure({
        Fabric.println("Hello {s} {s}", .{ user.name, user.lastname });
        Text(user.name, Style{});
    });
