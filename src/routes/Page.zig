...
var video_binded: Vapor.Binded = .{};

fn CodeRow(code: *SyntaxHighlighter, title: []const u8) void {
...
const PerformanceMonitor = struct {
    ...
    pub fn measure(self: *PerformanceMonitor) void {
        ...
        _ = Vapor.lib.runOnAnimationFrame(measure, .{self});
    }
...
var current_fetch: ?*Vapor.Fetch.Fetch = null;
pub fn init() void {
    const allocator = Vapor.persist.arena();

    highlighter = SyntaxHighlighter.init(allocator);
    highlighter.use_cpy_btn = false;
    highlighter.scrolling_enabled = true;
    ...
    form_highlighter.scrolling_enabled = true;
    ...
    reverb_highlighter.scrolling_enabled = true;
    ...
    reverb_middleware_highlighter.scrolling_enabled = true;
    ...
    websocket_highlighter.scrolling_enabled = true;
    ...
    complex_form_highlighter.scrolling_enabled = true;

    react_form_highlighter_modern = SyntaxHighlighter.init(allocator);
    react_form_highlighter_modern.use_cpy_btn = false;
    react_form_highlighter_modern.scrolling_enabled = true;

    current_fetch = fetch("/zig/VaporizeComplexForm.zig", .{ .method = .GET });
    current_fetch.?.handle(handlePageForm, .{});
    fetch("/tsx/ReactComplexFormModern.tsx", .{ .method = .GET }).handle(handlePageReactFormModern, .{});
...
fn handlePageForm(resp: Vapor.Fetch.Result) void {
    switch (resp) {
        .ok => |data| {
...
fn handlePageReactFormModern(resp: Vapor.Fetch.Result) void {
    switch (resp) {
        .ok => |data| {
...
const blocks: []const struct { title: []const u8, description: []const u8 } = &.{
    .{
        .title = "Vapor",
        .description = "Is a Compile-Time UI Engine, which takes a programmer first approach to building UIs <br> <span>ZIG → WASM → UI</span>",
    },
    .{
        .title = "Reverb",
...
fn boxes() void {
    ...
    const unit: f32 = 14;
    const height = unit * 4;
    // const navbar_top: f32 = 59; // y-position the navbar sits at; picked-up boxes pin here
    ...
    const box_style_base = Style{
        .size = .hw(.px(height), .px(height)),
        .visual = visual_base,
        .layout = .center,
        .transition = .{ .duration = 100 },
        .interactive = .{
            .hover = .{
                .transform = .distAndScale(.up, 4, 1.05),
    ...
    // 3. Pickup helper
    // =========================================================================
    // Once the navbar (sitting at `nav`) has scrolled down past a box's top
    // edge, switch the box to fixed so it rides along with the scroll. We pin
    // it at `nav` and keep its on-screen left so there's no sideways jump.
    // const Pickup = struct {
    //     fn maybe(base: Style, doc_top: f32, pinned_left: f32, scroll: f32, nav: f32) Style {
    //         return if (scroll > doc_top - nav)
    //             base.merge(.{ .position = .tl(.px(nav), .px(pinned_left), .fixed) })
    //         else
    //             base;
    //     }
    // };
    ...
    // =========================================================================
    // 4. Default (un-pinned) placements — same layout as before
    // =========================================================================
    var box_1_style = box_style_base.merge(.{
        .position = .tl(.px(-height), .px(-1), .absolute),
    });

    const top2 = unit * 28;
    var box_2_style = box_style_base.merge(.{
        .position = .tr(.px(top2), .px(-height), .absolute),
    });
    box_2_style.interactive.?.hover.?.transform = .distAndScale(.right, 4, 1.05);

    var box_3_style = box_style_base.merge(.{
        .position = .bl(.px(-height), .px(unit * 22), .absolute),
    });
    box_3_style.interactive.?.hover.?.transform = .distAndScale(.down, 4, 1.05);

    // =========================================================================
    // 5. Pick each box up as the navbar passes its top
    // =========================================================================
    // if (boxes_container.getOffsets()) |b| {
    //     // Document-space TOP edge of each box (container origin + in-container offset).
    //     const box_1_top = b.offset_top - height; // tl, top = -height
    //     const box_2_top = b.offset_top + top2; // tr, top =  top2
    //     const box_3_top = b.offset_top + b.offset_height; // bl, bottom = -height -> top sits at container bottom
    //
    //     // On-screen LEFT edge of each box, so pinning doesn't move it sideways.
    //     const box_1_left = b.offset_left; // tl, left ~ container left
    //     const box_2_left = b.offset_left + b.offset_width; // tr, right = -height -> left sits at container right
    //     const box_3_left = b.offset_left + unit * 22; // bl, left = unit*22
    //
    //     box_1_style = Pickup.maybe(box_1_style, box_1_top, box_1_left, current_scroll, navbar_top);
    //     box_2_style = Pickup.maybe(box_2_style, box_2_top, box_2_left, current_scroll, navbar_top);
    //     box_3_style = Pickup.maybe(box_3_style, box_3_top, box_3_left, current_scroll, navbar_top);
    // }

    // =========================================================================
    // 6. Render
    // =========================================================================
    Row().style(&box_1_style).children({
        Text("01").style(&mono_text_style).end();
    });

    Row().style(&box_2_style).children({
        Text("02").style(&mono_text_style).end();
    });

    Row().style(&box_3_style).children({
...
fn This() void {
    Html(
        ...
        \\<strong style="color: rgb(var(--text_color))"><i>~300kb</i></strong>
        \\.
...
fn toggleVideo() void {
    if (playing) {
        ...
        video_binded.playVideo();
        playing = true;
    }
}

const play_svg = @embedFile("play_svg.svg");

var current_scroll: f32 = 0;
fn trackScroll(evt: *Vapor.Event) void {
    if (Vapor.Kit.throttle(16)) return;

    current_scroll = evt.scrollTop();
    Vapor.cycle();
}
// var boxes_container: Vapor.Binded = .{};
pub fn render() void {
    Stack()
        .scroll(.scroll_y())
        .width(.percent(100))
            ...
            if (Vapor.isDesktop()) {
                Row().style(&.{
                    ...
                    Stack().style(&.{ .layout = .bottom_left }).children({
                        ...
                        Row().style(&.{ .margin = .t(12), .layout = .x_even_center }).children({
        ...
        Stack()
            ...
            Row()
                ...
                Stack().width(.mobile_desktop_percent(100, 40))
                    ...
                    CodeRow(&complex_form_highlighter, "vaporize.zig");
                });
                ...
                Stack().width(.mobile_desktop_percent(100, 40))
                    ...
                    CodeRow(&react_form_highlighter_modern, "react-hook-zod-shad.zig");
                });
            });
        });
                ...
                Row()
                    ...
                    Row()
                        ...
                        Button(toggleVideo, .{})
                            ...
                            .children({
                            if (!playing) {
                                Svg(.{ .svg = play_svg, .override = true })
                                    .fill(.palette(.text_color))
                                    .stroke(.palette(.text_color))
                                    .width(.px(48))
                                    .height(.px(48))
        ...
        Row().style(&.{
            ...
            Stack().style(&.{
                ...
                Row().spacing(16).width(.percent(100)).layout(.center).children({
        ...
        Stack().style(&.{
            ...
            Row().style(&.{
                ...
                Row()
                    ...
                    // .border(.simple(.palette(.text_color)))
                    .children({
                    CodeRow(&reverb_highlighter, "reverb.zig");
        ...
        Stack().style(&.{
            ...
            Row().style(&.{
                ...
                Row()
                    ...
                    .width(.mobile_desktop_percent(100, 40)).height(.mobile_desktop_percent(40, 100))
                    .children({
                    CodeRow(&reverb_middleware_highlighter, "reverb_middleware.zig");
                });
            });
        });
        ...
        Row().style(&.{
            ...
            Row().style(&.{
                ...
                Row()
                    ...
                    .width(.mobile_desktop_percent(100, 40)).height(.mobile_desktop_percent(40, 100))
                    .children({
                    CodeRow(&websocket_highlighter, "websocket.zig");
                });
        ...
        Row().style(&.{
            ...
            Stack().style(&.{
                ...
                Stack().style(&.{ .child_gap = 16 }).children({
                    ...
                    RedirectLink(.{ .url = "https://assembler.kyber.run", .aria_label = "Assembler website" }).style(&.{
                        .visual = .{ .text_color = .white, .text_decoration = .none },
                    }).children({
                        Text("Assembler").hover(.{ .text_decoration = .underline }).duration(100).baseStyle(&Styles.muted_text).end();
                    });
                    ...
                    RedirectLink(.{ .url = "https://vapor-ui.com", .aria_label = "Vapor UI website" }).style(&.{
                        .visual = .{ .text_color = .white, .text_decoration = .none },
                    }).children({
            ...
            Stack().style(&.{
                ...
                Row().style(&.{ .child_gap = 8, .layout = .left_center }).children({
...
pub fn VideoStack(active: *bool) Vapor.ButtonBuilder(.static) {
    return Button(toggle_expand, .{active})
        .direction(.column)
        .height(.percent(70))
        .width(.percent(24))
