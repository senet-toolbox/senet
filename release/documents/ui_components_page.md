...
fn alertContent(_: *Alert) void {
    Stack().spacing(16).children({
        ...
        Box().layout(.right_center).spacing(16).children({
            Button(Alert.close, .{&alert}).children({
                Text("Cancel").end();
            });
...
fn render() void {
    if (is_open) {
        Vapor.Hooks(.mounted, mount, .{})({
            Vapor.Hooks(.destroy, destroy, .{})({
                // Dialog content
            });
        });
    }
...
var background: Vapor.Types.Color = .palette(.background);
...
var tint: Vapor.Types.Color = .transparentizeHex(.palette(.tint), 0.8);
...
```zig
fn CommonButton(func: anytype, args: anytype) Vapor.Builder(.pure) {
    return Button(func, args)
        .cursor(.pointer)
...
fn CheckBox(func: anytype, args: anytype) Vapor.Builder(.pure) {
    return Button(func, args)
        .width(.px(20))
        .height(.px(20))
        .cursor(.pointer)
