...
These are the most common component type you will use in your applications. They declare their variables globally, but are only available within the file they are declared in.
...
// *Counter here is a reference to the instance,
// that we can operate on different instances
...
`comptime` is an incredibly powerful tool, that is part of the language, and can be used to generate components of different types.
...
For example, we can use `comptime` to generate a `Counter` for various value types. The same `comptime` system powers the `DataTable` component in Vapor.

##### This is like function components in React, Solid, or other such frameworks.

A Function Component is a function that returns a `type`. Each distinct instantiation — a unique combination of arguments — produces its own type with its own local variables.
...
##### Each instantiation has independent state — but only if the instantiation is distinct.

`Counter(i32, -1)` and `Counter(u32, 1)` are two different types, so each has its own `count`.

⚠️ Zig memoizes generic instantiation: calling `Counter(i32, 0)` in two different places returns the **same** type, which means both share a single `count`. The arguments are what make an instantiation unique, not the call site. If you need two counters of the same type and starting value to be independent, reach for an Instance Component instead — its state is bound to the struct value, so every `.{}` you create is genuinely separate.
...
inside all our functions. In the Zig version, above we can just treat the variable as a normal, akin to the global component, but with all the benefits of local bounded variables.
...
```zig
const dirty_nodes = Vapor.lib.dirty_nodes;
const added_nodes = Vapor.lib.added_nodes;

for (dirty_nodes.items) |node| {
    // Do something with the dirty node
}

// Removals are queued separately, so their exit animations can run before
